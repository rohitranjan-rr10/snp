<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (!empty($_FILES["fileToUpload"]["name"])) {
            ...
            $target_file = $_SERVER['DOCUMENT_ROOT'] . $upload_directory . $current_datetime . "_" . $sanitized_file_name;
            ...
            if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
                $updateInvoiceSQL = "UPDATE master_data SET invoice = ? WHERE serial_number = ?";
                $stmtInvoice = $conn->prepare($updateInvoiceSQL);
                $stmtInvoice->bind_param("si", $file_url, $serialNumber);
                $stmtInvoice->execute();
                $stmtInvoice->close();
            }
        } else {
            $selectOldFileSQL = "SELECT invoice FROM master_data WHERE serial_number = ?";
            $stmtOldFile = $conn->prepare($selectOldFileSQL);
            $stmtOldFile->bind_param("i", $serialNumber);
            $stmtOldFile->execute();
            $stmtOldFile->store_result();

            if ($stmtOldFile->num_rows > 0) {
                $stmtOldFile->bind_result($oldFileURL);
                $stmtOldFile->fetch();
                $updateInvoiceSQL = "UPDATE master_data SET invoice = ? WHERE serial_number = ?";
                $stmtInvoice = $conn->prepare($updateInvoiceSQL);
                $stmtInvoice->bind_param("si", $oldFileURL, $serialNumber);
                $stmtInvoice->execute();
                $stmtInvoice->close();
            }
            $stmtOldFile->close();
        ...
        $sql = "UPDATE master_data SET indentor=?, notesheet_purchase_order_no=?, purchase_order_date=?, description=?, nature_of_asset=?, quantity=?, gross_amount=?, bill_no=?, bill_date=?, supplier=?, date_of_receipt=?, date_of_installation=?, department=?, location=? WHERE serial_number=?";

        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ssssssssssssssi", $indentor, $purchase_order_no, $purchase_order_date, $descriptionJSON, $nature_of_assetJSON, $quantityJSON, $gross_amountJSON, $bill_no, $bill_date, $supplier, $date_of_receipt, $date_of_installation, $department, $location, $serialNumber);
